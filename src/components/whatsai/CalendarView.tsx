  ...
  return <>
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_330px]">
      <Card className="overflow-hidden border-[#d8dee4] bg-white">
        <CardHeader className="flex flex-col gap-4 border-b border-[#d8dee4] p-4 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle className="text-lg">{view === 'month' ? format(cursor, 'MMMM yyyy') : `Week of ${format(days[0], 'd MMM')}`}</CardTitle><p className="mt-1 text-xs text-muted-foreground">{appointments.length} appointment{appointments.length === 1 ? '' : 's'} in your schedule</p></div><div className="hidden items-center gap-2 sm:flex"><Button variant="outline" size="icon" onClick={() => move(-1)} aria-label="Previous period"><ChevronLeft className="h-4 w-4" /></Button><Button variant="outline" size="sm" onClick={() => { setCursor(new Date()); setSelectedDate(new Date()); }}>Today</Button><Button variant="outline" size="icon" onClick={() => move(1)} aria-label="Next period"><ChevronRight className="h-4 w-4" /></Button><Tabs value={view} onValueChange={(value) => setView(value as 'month' | 'week')}><TabsList className="ml-1"><TabsTrigger value="month">Month</TabsTrigger><TabsTrigger value="week">Week</TabsTrigger></TabsList></Tabs></div><div className="text-xs font-medium text-[#087d70] sm:hidden">Upcoming agenda</div></CardHeader>
        <CardContent className="p-3 sm:p-5">
          <div className="space-y-2 sm:hidden">
            {agenda.length ? agenda.map((appointment) => (
              <button key={appointment.id} type="button" onClick={() => openAppointment(appointment)} className="wa-row flex w-full items-center gap-3 rounded-xl border border-[#e2e7e4] p-3 text-left">
                <div className="flex h-12 w-12 shrink-0 flex-col items-center justify-center rounded-xl bg-[#edf8f4] text-[#075e54]"><span className="text-[10px] font-semibold uppercase">{format(parseISO(appointment.scheduled_at), 'MMM')}</span><span className="text-lg font-semibold leading-none">{format(parseISO(appointment.scheduled_at), 'd')}</span></div>
                <div className="min-w-0 flex-1"><div className="truncate text-sm font-semibold text-[#111b21]">{appointment.contactName}</div><div className="mt-1 text-xs text-[#667781]">{format(parseISO(appointment.scheduled_at), 'h:mm a')} · {appointment.appointment_type.replace('_', ' ')}</div></div>
                <Badge variant="success">{appointment.status}</Badge>
              </button>
            )) : <div className="rounded-xl bg-[#f7faf8] p-8 text-center"><CalendarDays className="mx-auto h-7 w-7 text-[#00a884]" /><p className="mt-3 text-sm font-semibold">No appointments yet</p><p className="mt-1 text-xs text-[#667781]">Bookings made from WhatsApp will appear here.</p></div>}
          </div>
          <div className="hidden sm:block"><div className="grid grid-cols-7 border-b border-[#d8dee4] pb-2 text-center text-[10px] font-semibold uppercase tracking-wide text-[#667781]">{['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => <div key={day}>{day}</div>)}</div><div className="mt-2 grid grid-cols-7 gap-2">{days.map((day) => { const key = format(day, 'yyyy-MM-dd'); const count = appointments.filter((appointment) => format(parseISO(appointment.scheduled_at), 'yyyy-MM-dd') === key).length; return <button type="button" key={key} onClick={() => setSelectedDate(day)} className={cn('relative min-h-20 rounded-xl border border-transparent p-2 text-left transition hover:border-[#00a884] lg:min-h-24', !isSameMonth(day, cursor) && view === 'month' && 'text-slate-300', isSameDay(day, selectedDate) && 'border-[#00a884] bg-[#e7fce3] text-[#075e54]')}><span className={cn('flex h-7 w-7 items-center justify-center rounded-full text-sm font-semibold', isSameDay(day, new Date()) && 'bg-[#075e54] text-white')}>{format(day, 'd')}</span>{appointmentDays.has(key) ? <span className="absolute bottom-2 left-2 flex items-center gap-1 text-[10px] font-semibold text-[#075e54]"><i className="h-2 w-2 rounded-full bg-[#00a884]" />{count > 1 ? count : ''}</span> : null}</button>; })}</div></div>
        </CardContent>
      </Card>
      <Card className="hidden border-[#d8dee4] bg-white xl:block"><CardHeader className="border-b border-[#d8dee4] p-4"><CardTitle className="flex items-center gap-2 text-base"><CalendarDays className="h-4 w-4 text-[#00a884]" />{format(selectedDate, 'EEEE, d MMM')}</CardTitle></CardHeader><CardContent className="space-y-3 p-4">{dayAppointments.length ? dayAppointments.map((appointment) => <AppointmentCard key={appointment.id} appointment={appointment} onClick={() => openAppointment(appointment)} />) : <div className="rounded-xl bg-[#f7faf8] p-6 text-center"><CalendarDays className="mx-auto h-7 w-7 text-slate-400" /><p className="mt-3 text-sm font-semibold">Nothing booked for this day</p><p className="mt-1 text-xs text-muted-foreground">AI-booked visits and calls will appear here.</p></div>}</CardContent></Card>
    </div>
