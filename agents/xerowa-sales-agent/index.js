...
app.post('/book-visit', async (req, res) => {
  ...
  if (supabase) {
    ...
    if (businessId && !contactId) {
      const contact = await supabase
        ...
        .select('id')
        .eq('business_id', businessId)
        .or(`lead_id.eq.${payload.lead_id},phone.eq.${payload.phone}`)
        .limit(1)
        .maybeSingle();
      contactId = contact.data?.id || null;
    }

    if (businessId && contactId && !threadId) {
    ...
    if (businessId && contactId) {
      const scheduledAt = new Date(`${payload.scheduled_date}T${payload.scheduled_time}:00+05:30`).toISOString();
      ...
      const values = {
        ...
        contact_id: contactId,
        thread_id: threadId,
        title: `Site visit with ${payload.lead_name}`,
        appointment_type: 'site_visit',
        scheduled_at: scheduledAt,
        status: 'scheduled',
        notes: 'Booked by the tenant-scoped real-estate assistant.',
      };
      const appointmentWrite = existing.data
        ? await supabase.from('appointments').update(values).eq('id', existing.data.id).eq('business_id', businessId).select().single()
        : await supabase.from('appointments').insert(values).select().single();
      if (appointmentWrite.error) {
        return res.status(500).json({ ok: false, error: appointmentWrite.error.message });
      }
      appointment = appointmentWrite.data;
      record = {
        ...appointment,
      ...
      if (threadId) {
        await supabase.from('handoff_events').insert({
          ...
          priority: 'high',
          status: 'pending',
          summary: `${payload.lead_name} booked ${payload.scheduled_date} at ${payload.scheduled_time}.`,
        });
      }
    } else {
  ...
  await logAgentRun({
    ...
    projectId: payload.project_id,
    input: payload,
    output: { appointment, record, confirmation, reminderSteps, outbound, business_id: businessId },
  });
