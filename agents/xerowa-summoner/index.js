...
const TIMEOUT_FALLBACK_MESSAGE = 'Abhi thoda delay ho raha hai. Aapka message mil gaya hai; team jaldi reply karegi.';
...
// ─────────────────────────────────────────────────────────────────────────────
// Compatibility helpers for pre-generic lead records. New routing uses the
// business/channel/playbook model above.
// ─────────────────────────────────────────────────────────────────────────────
...
function resolveLegacyLeadContext(lead) {
  return {
    builderId: lead?.builder_id || DEFAULT_BUILDER_ID || null,
    projectId: lead?.project_id || DEFAULT_PROJECT_ID || null,
  };
}
...
function normalizeRouteInput(payload) {
  ...
  return {
    ...
    projectId: payload.project_id || payload.lead?.project_id || DEFAULT_PROJECT_ID || null,
...
async function handleInboundWhatsAppMessage(message, envelope) {
  ...
  // Legacy lead rows remain readable during the database migration window, but
  // runtime routing is business/playbook based and never dispatches to product-specific modules.
  let lead = await findLeadByPhone(phone);
  let { builderId, projectId } = resolveLegacyLeadContext(lead);
  builderId = businessId || builderId;
  ...
  // Keep the compatibility message write until the legacy schema migration is
  // completed in its own reviewed change.
  if (builderId) {
